GridGame.api_ui = {
  ...
  init_button_actions: function () {
    ...
    $('.button[data-action="reset"]').on('click', function () {
      GridGame.reset();
      if ($('input[data-action="change-speed"]').val()) {
        GridGame.turn_time = $('input[data-action="change-speed"]').val();
      }
    });
    $('input[data-action="change-speed"]').on('change', function () {
      GridGame.turn_time = $(this).val();
      GridGame.stop();
      GridGame.start();
    });
    $('.button[data-action="step"]').on('click', function () {
      GridGame.turn();
    });
  }
};
